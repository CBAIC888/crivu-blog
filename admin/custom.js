...
const resources = [
  ...
  ['media','媒體庫'],['likes','按讚數據'],['guestbook','留言板'],['settings','設定'],
];
const statusNames = {
...
const renderLikes = (surface) => {
  const rows=state.items.map((item)=>`<article class="like-row" data-like-row data-search="${escapeHtml(`${item.title} ${item.slug}`.toLowerCase())}"><div><strong>${escapeHtml(item.title)}</strong><small>/${escapeHtml(item.slug)} · ${escapeHtml(displayDate(item.publishedAt))}</small></div><div><input type="number" min="0" step="1" value="${Number(item.likeCount||0)}" data-like-count="${escapeHtml(item.id)}" aria-label="《${escapeHtml(item.title)}》按讚數"/><button data-like-save="${escapeHtml(item.id)}">儲存</button></div></article>`).join('')||'<p class="empty">暫無文章</p>';
  surface.innerHTML=`<div class="list"><div class="list-head list-head--single"><input type="search" placeholder="搜尋文章" data-like-search/></div><div>${rows}</div></div>`;
  surface.querySelector('[data-like-search]').addEventListener('input',(event)=>{const query=event.target.value.trim().toLowerCase();surface.querySelectorAll('[data-like-row]').forEach((row)=>{row.hidden=Boolean(query&&!row.dataset.search.includes(query));});});
  surface.querySelectorAll('[data-like-save]').forEach((button)=>button.addEventListener('click',async()=>{const id=button.dataset.likeSave,input=surface.querySelector(`[data-like-count="${id}"]`),likeCount=Math.max(0,Math.floor(Number(input.value)||0));button.disabled=true;try{const data=await api(`likes/${encodeURIComponent(id)}`,{method:'PUT',body:JSON.stringify({likeCount})});input.value=String(data.likeCount);notify('按讚數已更新');}catch(error){notify(error.message,true);}finally{button.disabled=false;}}));
};

const renderList = () => {
  const surface=document.querySelector('[data-surface]');
  if(state.resource==='media'){renderMedia(surface);return;}
  if(state.resource==='likes'){renderLikes(surface);return;}
...
const openResource = async (resource) => {
  ...
  const newButton=document.querySelector('[data-new]');newButton.hidden=['settings','media','likes'].includes(resource);newButton.textContent=resource==='guestbook'?'新增捧場':'新增';
  notify('載入中…');
  try{
...
const pageForm = (item) => `<div class="editor-grid"><form class="form" data-form><div class="form-row">${label('標題','title',input('title',item.title,'text','required'))}${label('狀態','status',select('status',item.status||'draft',[['draft','草稿'],['published','已發布'],['archived','封存']]))}</div>${label('Slug','slug',input('slug',item.slug,'text','required'))}<div class="editor-toolbar"><button type="button" data-wrap="**">粗體</button><button type="button" data-prefix="## ">標題</button><button type="button" data-prefix="> ">引用</button><button type="button" data-upload-image>上傳並插入圖片</button><input type="file" accept="image/*" data-image-file hidden/><button type="button" data-fullscreen>全螢幕</button></div>${label('正文','bodyMarkdown',textarea('bodyMarkdown',item.bodyMarkdown,22,'data-body'))}<details><summary>SEO</summary>${label('SEO 標題','seoTitle',input('seoTitle',item.seoTitle))}${label('SEO 描述','seoDescription',textarea('seoDescription',item.seoDescription,3))}</details></form><aside class="preview" data-preview></aside></div>`;
...
try{const session=await api('session');shell(session.user);await openResource('articles');}catch{app.innerHTML=`<main class="login"><div><a class="brand" href="/articles"><img src="/assets/img/logo.svg" alt="CRIVU"></a><h1>內容管理</h1><p>使用獲准的郵箱接收一次性驗證碼後登入。</p><a class="login-button" href="/cdn-cgi/access/logout">重新驗證郵箱</a></div></main>`;}
