...
export const onRequest = handle(async (context) => {
  ...
  if(resource==='guestbook'){
    if(method==='GET'){const status=url.searchParams.get('status');const result=status&&status!=='all'?await db.prepare(`SELECT * FROM guestbook_entries WHERE status=? ORDER BY created_at DESC LIMIT 300`).bind(status).all():await db.prepare(`SELECT * FROM guestbook_entries ORDER BY created_at DESC LIMIT 300`).all();return json({items:rows(result).map(x=>({id:x.id,authorName:x.author_name,body:x.body,status:x.status,source:x.source,adminReply:x.admin_reply||'',createdAt:x.created_at}))});}
    if(method==='PATCH'&&id){const p=await parseJson(context.request),status=allowed(p.status,['pending','approved','hidden','spam'],'pending');await db.prepare(`UPDATE guestbook_entries SET status=?,admin_reply=?,approved_at=CASE WHEN ?='approved' THEN COALESCE(approved_at,?) ELSE approved_at END,updated_at=? WHERE id=?`).bind(status,cleanText(p.adminReply,1200),status,now(),now(),id).run();return json({saved:true});}
