...
const iso = (value) => String(value || '').match(/^\d{4}-\d{2}-\d{2}/)?.[0] || null;
const published = (item) => item.published !== false;
const statements = ['PRAGMA foreign_keys = ON;', 'BEGIN;', 'PRAGMA defer_foreign_keys = ON;'];
...
for (const post of posts) {
  ...
  const metadata = { category: post.category || '', legacyIssue: post.issue || '' };
...
const researchCover = '/assets/img/projects/world-word-history-cover.webp';
const researchCoverId = rememberMedia(researchCover, '對「世界」的探索');
add(`INSERT INTO articles (id,type,status,slug,title,summary,body_markdown,language,cover_media_id,cover_url,published_at,metadata_json) VALUES ('article_world_zh','research','published','world-word-exploration','對『世界』的探索','從先秦兩漢的舊字、東漢譯經與佛教宇宙論出發，追索「世界」一詞近兩千年的形成與變化。',${q(researchZh)},'zh-Hant',${q(researchCoverId)},${q(researchCover)},'2026-07-23',${q(JSON.stringify({ format: 'html', projectSlug: 'world-word-history', gallery: '/records/world-word-history/gallery', exhibition: '/records/world-word-history/museum' }))}) ON CONFLICT(id) DO UPDATE SET body_markdown=excluded.body_markdown,metadata_json=excluded.metadata_json`);
add(`INSERT INTO articles (id,type,status,slug,title,summary,body_markdown,language,cover_media_id,cover_url,published_at,metadata_json) VALUES ('article_world_en','research','published','world-word-exploration-en','An Exploration of Shijie 世界 (“World”)','Tracing nearly two millennia in the formation and transformation of shijie.',${q(researchEn)},'en',${q(researchCoverId)},${q(researchCover)},'2026-07-23',${q(JSON.stringify({ format: 'markdown', isTranslation: true, projectSlug: 'world-word-history', gallery: '/records/world-word-history/gallery?lang=en', exhibition: '/records/world-word-history/museum' }))}) ON CONFLICT(id) DO UPDATE SET body_markdown=excluded.body_markdown,metadata_json=excluded.metadata_json`);
...
const galleryFiles = fs.readdirSync(path.join(root, 'src/assets/world-gallery')).filter((filename) => filename.endsWith('.webp')).sort();
for (const filename of galleryFiles) {
  const name = path.basename(filename, '.webp');
  const id = `media_world_${name.replaceAll('-', '_')}`;
  media.set(id, { id, filename, key: `research/world-gallery/${filename}`, publicUrl: `/assets/world-gallery/${filename}`, title: name.replaceAll('-', ' ') });
}
...
statements.push('COMMIT;', `-- ${JSON.stringify({ sourceHash, ...summary })}`);
