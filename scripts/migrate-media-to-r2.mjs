...
const roots=[['assets/img','site'],['src/assets/world-gallery','research/world-gallery']];
...
for(const item of items){const result=spawnSync('npx',['wrangler','r2','object','put',`${bucket}/${item.key}`,'--file',item.file],{cwd:root,stdio:'inherit'});if(result.status!==0)throw new Error(`Upload failed: ${item.key}`);}
