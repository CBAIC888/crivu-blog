...
const shell = ({ current, title, description, path, origin, main, image='', language='zh-Hant', extraScripts='', assetVersion='' }) => `${head({title,description,canonicalPath:path,origin,image,language,assetVersion})}<body data-current="${current}"${language==='en'?' data-language="en"':''}>${main}<script defer src="${escapeHtml(versioned('/assets/academic/analytics.js',assetVersion))}"></script><script type="module" src="${escapeHtml(versioned('/assets/academic/public-site.js',assetVersion))}"></script>${extraScripts}</body></html>`;
